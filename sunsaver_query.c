...
int main(int argc, char**argv)
{
	...
	uint16_t data[50];
        ...
        while ((i = getopt(argc, argv, "p:")) != -1)
        {
          ...
          {
            case 'p': // port 
                      sscanf(optarg, "%x", &port);
                      break;
            default:
                      fprintf(stderr, "usage: %s [-p keyspan_port_number]\n",
                              *argv);
                      exit(-1);
          }
        }
        ...
        switch(port)
        {
#ifdef RASPPI
          case 0:
	         modBus = modbus_new_rtu("/dev/ttyUSB0", 9600, 'N', 8, 2);
                 break;
          case 1:
	         modBus = modbus_new_rtu("/dev/ttyUSB1", 9600, 'N', 8, 2);
                 break;
          case 2:
	         modBus = modbus_new_rtu("/dev/ttyUSB2", 9600, 'N', 8, 2);
                 break;
          case 3:
	         modBus = modbus_new_rtu("/dev/ttyUSB3", 9600, 'N', 8, 2);
                 break;
          case 4:
	         modBus = modbus_new_rtu("/dev/ttyUSB4", 9600, 'N', 8, 2);
                 break;
          case 5:
	         modBus = modbus_new_rtu("/dev/ttyUSB5", 9600, 'N', 8, 2);
                 break;
          case 6:
	         modBus = modbus_new_rtu("/dev/ttyUSB6", 9600, 'N', 8, 2);
                 break;
          case 7:
	         modBus = modbus_new_rtu("/dev/ttyUSB7", 9600, 'N', 8, 2);
                 break;
          default:
	         modBus = modbus_new_rtu("/dev/ttyUSB3", 9600, 'N', 8, 2);
                 break;
          ...
          case 0:
	         modbus_init_rtu(&mb_param, "/dev/ttyUSB0", 9600, "none", 8, 2);
                 break;
          case 1:
	         modbus_init_rtu(&mb_param, "/dev/ttyUSB1", 9600, "none", 8, 2);
                 break;
          case 2:
	         modbus_init_rtu(&mb_param, "/dev/ttyUSB2", 9600, "none", 8, 2);
                 break;
          case 3:
	         modbus_init_rtu(&mb_param, "/dev/ttyUSB3", 9600, "none", 8, 2);
                 break;
          default:
	         modbus_init_rtu(&mb_param, "/dev/ttyUSB3", 9600, "none", 8, 2);
                 break;
...
/*
	...
	read_input_registers(&mb_param, SUNSAVERMPPT, 0x0008, 5, data);
...
*/
        ...
        //modbus_read_input_registers(modBus, SUNSAVERMPPT, 45, data);
        modbus_read_registers(modBus, 0x0008, 45, data);
	
	/* Close the MODBUS connection */
	modbus_close(modBus);
#endif
#ifndef RASPPI
        read_input_registers(&mb_param, SUNSAVERMPPT, 0x0008, 45, data);
	...
	adc_vb_f=data[0]*100.0/32768.0;
	printf("adc_vb_f [ Battery Voltage, Filtered ] %x = %.2f V\n",data[0], adc_vb_f);
	
	adc_va_f=data[1]*100.0/32768.0;
	printf("adc_va_f [ Array Voltage, Filtered ] = %.2f V\n",adc_va_f);
	
	adc_vl_f=data[2]*100.0/32768.0;
	printf("adc_vl_f [ Load Voltage, Filtered ] = %.2f V\n",adc_vl_f);
	
	adc_ic_f=data[3]*79.16/32768.0;
	printf("adc_ic_f [ Charging Current, Filtered ] = %.2f A\n",adc_ic_f);
	
	adc_il_f=data[4]*79.16/32768.0;
	printf("adc_il_f [ Load Current, Filtered ] = %.2f A\n",adc_il_f);
	
	T_hs=data[5];
	printf("T_hs [ Heatsink Temp ] %x = %d °C\n",data[5],T_hs);
	
	T_batt=data[6];
	printf("T_batt [ Battery Temp -- ambient ] %x = %d °C\n",data[6], T_batt);
	
	T_amb=data[7];
	printf("T_amb [ Ambient Temp ] %x = %d °C\n",data[7], T_amb);
	
	T_rts=data[8];
	printf("T_rts [ Remote battery Temp Sensor 0x08 if not connected ] %x = %d °C\n",data[8], T_rts);
	
	charge_state=data[9];
        printf("Charge State Value = %x\n", charge_state);
	switch (charge_state) {
		case 0:
	...
	printf("Vb_f [ Battery Voltage 25s filter ] = %.2f V\n",Vb_f);
	
	Vb_ref=data[12]*96.667/32768.0;
	printf("Vb_ref [ Battery regulator ref voltage ] = %.2f V\n",Vb_ref);
	
	Ahc_r=((data[13] << 16) + data[14])*0.1;
	printf("Ahc_r [ Amp Hour Charge - Resetable ] = %.2f Ah\n",Ahc_r);
	
	Ahc_t=((data[15] << 16) + data[16])*0.1;
	printf("Ahc_t [ Amp Hour Charge - Total ] = %.2f Ah\n",Ahc_t);
	
	kWhc=data[17]*0.1;
	printf("kWhc [KWh charge] = %.2f kWh\n",kWhc);
	...
	printf("V_lvd [ Load current compensated LVD Voltage ] = %.2f V\n",V_lvd);
	
	Ahl_r=((data[21] << 16) + data[22])*0.1;
	printf("Ahl_r [ Amp Hour Load - Resetable ] = %.2f Ah\n",Ahl_r);
	
	Ahl_t=((data[23] << 16) + data[24])*0.1;
	printf("Ahl_t [ Amp Hour Load - Total ] = %.2f Ah\n",Ahl_t);
	
	hourmeter=(data[25] << 16) + data[26];
	printf("hourmeter = %d h\n",hourmeter);
	
	alarm=(data[27] << 16) + data[28];
	printf("alarm = Controller self-diagnostic alarms %x - %x:\n", data[27], data[28]);
	if (alarm == 0) {
		printf("\tNo alarms\n");
	} else {
	...
	dip_switch=data[29];
	printf("dip_switch = DIP switch settings: %x\n", data[29]);
	if (dip_switch & 1) {
	...
	printf("Power_out [ Charge output power ] = %.2f W\n",Power_out);
	
	Sweep_Vmp=data[32]*100.0/32768.0;
	printf("Sweep_Vmp [ Array Vmp found during sweep ] = %.2f V\n",Sweep_Vmp);
	
	Sweep_Pmax=data[33]*989.5/65536.0;
	printf("Sweep_Pmax [ Array Pmax found during sweep ] = %.2f W\n",Sweep_Pmax);
	
	Sweep_Voc=data[34]*100.0/32768.0;
	printf("Sweep_Voc [ Array Voc found during sweep ] = %.2f V\n",Sweep_Voc);
	...
	printf("Ahc_daily [ Amp Hour Charge - Daily ] = %.2f Ah\n",Ahc_daily);
	
	Ahl_daily=data[38]*0.1;
	printf("Ahl_daily [ Amp Hour Load - Daily ] = %.2f Ah\n",Ahl_daily);
	...
	printf("vb_min [ Min Battery Voltage ] = %.2f V\n",vb_min);
	
	vb_max=data[44]*100.0/32768.0;
	printf("vb_max [ Max Battery Voltage ] = %.2f V\n",vb_max);

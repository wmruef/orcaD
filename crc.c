 ...
 *
 * crc.c : Cyclic Redundancy Check routines  
 *
 * Created: August 2005
 *
...
int crc16AddByte ( unsigned char data, unsigned int crc )
{
  return ccitt_16[ (unsigned char)(( crc >> 8 ) ^ data ) ] ^ ( crc << 8 );
}
...
int crc16AddData ( unsigned char *data, long numBytes, unsigned int crc )
{
  ...
  int newCrc = 0;
  if ( numBytes > 0 ) {
    newCrc = crc16AddByte( data[0], crc );
    for ( i=1; i < numBytes; i++ ){ 
      newCrc = crc16AddByte( data[i], newCrc );
    }
  }
  return newCrc;
}
